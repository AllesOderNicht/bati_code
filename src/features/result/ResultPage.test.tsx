...
describe("ResultPage view model", () => {
  ...
  it("creates a valid ResultViewModel with all fields", () => {
    ...
    expect(viewModel.characterImageSrcSet).toContain("960w");
    expect(viewModel.characterImageSrcSet).toContain("1440w");
    expect(viewModel.characterImageSizes).toContain("100vw");
    expect(viewModel.characterImageDownloadSrc).toBeTruthy();
    expect(viewModel.characterImageDownloadFilename).toBe("penguin_meituan.png");
  ...
  it("renders the four-section MBTI-style result layout", () => {
    ...
    const image = screen.getByRole("img", { name: "爱喝开水的企鹅角色插画" });
    ...
    expect(screen.getByLabelText("相关公司")).toBeInTheDocument();
  ...
  it("renders bottom actions with share button, save hint, and repo link", () => {
    const viewModel = createResultViewModel(sampleResult, sampleExplanation);
    ...
    expect(screen.getByText("长按结果卡保存图片")).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "回到首页" }),
    ).toBeInTheDocument();
    expect(screen.getByText("项目已开源：")).toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: "GitHub · AllesOderNicht/bati_code" }),
    ).toHaveAttribute("href", "https://github.com/AllesOderNicht/bati_code");
  });
  ...
  it("renders SSR badge and exclusive note", () => {
    ...
    expect(screen.getByLabelText("SSR 稀有人设")).toBeInTheDocument();
    expect(screen.getByText("✦ 天选打工人 ✦")).toBeInTheDocument();
  });
  ...
  it("renders concentration bar and exclusive note for concentration results", () => {
    ...
    expect(screen.getByText("99.99%浓度的阿里人")).toBeInTheDocument();
    expect(screen.getByText("为什么像")).toBeInTheDocument();
  });
});
