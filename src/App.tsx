...
import { preloadImage } from "./features/result/preloadImage";
...
const BATI_LEGACY_GALLERY_ROUTE = "/bati/gallery";
...
function AppShell({ title, questions }: AppShellProps) {
  ...
  useEffect(() => {
    const isLastQuestion =
      screen === "quiz" && currentIndex === questions.length - 1;

    if (!isLastQuestion || resultPageState.status !== "ready") {
      return;
    }

    preloadImage(
      resultPageState.result?.characterImageSrc,
      resultPageState.result?.characterImageSrcSet,
      resultPageState.result?.characterImageSizes,
    );
  }, [currentIndex, questions.length, resultPageState, screen]);
